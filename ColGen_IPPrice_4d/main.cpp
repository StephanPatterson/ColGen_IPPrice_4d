...
//  ColGen_IPPrice_4d
//
//  Created by StephanP on 4/28/23.
//  Copyright © 2023 Stephan Patterson. All rights reserved.
//
//  Version for extending the location vectors to four dimensions instead of 2
//  Comparing running times to 2d version
...
int main(int argc, const char * argv[]) {
   ...
   int Pnum =10;//If NumMoMeas = 1, Number of months to process. Will go consecutively from start month, skipping any empty months.
   std::string startmonth = "Apr";
   int startyear = 14;
   bool namevars = 0;
   ...
   for (int i = 0; i < Pnum; ++i)
   {
      ...
      for (int j = 0; j < NumMoMeas; ++j)
      {
         while (month == currentmonth )
         {
            ...
            // also adding a different number so that coordinates 3 & 4 are different from 1 & 2
            if (same)
               PsuppT[i].push_back(SuppPt(loc1+115, loc2, loc1+115, loc2, 1.0, totalsupp));
            else
               PsuppT[i].push_back(SuppPt(loc1+115, loc2, loc1+120, loc2+5, 1.0, totalsupp));
            ++Psnum[i];
            ++totalsupp;
...
//      std::cout << "Minimum mass is " << minmass <<std::endl;
      ...
      for (int i = 0; i < Pnum; ++i)
      {
         ...
         sum3 += lambda[i]*PsuppT[i][indices[i]].loc3;
         sum4 += lambda[i]*PsuppT[i][indices[i]].loc4;
      ...
      double newc = 0;
      SuppPt Pbar0 = SuppPt(sum1, sum2, sum3, sum4,  0.0);
      for (int i = 0; i < Pnum; ++i)
      {
         double diff1 =Pbar0.loc1-PsuppT[i][indices[i]].loc1;
         double diff2 =Pbar0.loc2-PsuppT[i][indices[i]].loc2;
         double diff3 =Pbar0.loc3-PsuppT[i][indices[i]].loc3;
         double diff4 =Pbar0.loc4-PsuppT[i][indices[i]].loc4;
         newc += lambda[i]*(diff1*diff1+diff2*diff2+diff3*diff3+diff4*diff4);
...
double compute_c(const long int &S0, const int &Pnum, const int * Psnum, const std::vector< std::vector<SuppPt> > &Psupp, const long int &h, const double * lambda)
{
   ...
   sum3 += lambda[0]*Psupp[0][index].loc3;
   sum4 += lambda[0]*Psupp[0][index].loc4;
   for (int i = 1; i < Pnum-1; ++i)
   {
      ...
      sum3 += lambda[i]*Psupp[i][index].loc3;
      sum4 += lambda[i]*Psupp[i][index].loc4;
   }
   ...
   sum3 += lambda[Pnum-1]*Psupp[Pnum-1][index].loc3;
   sum4 += lambda[Pnum-1]*Psupp[Pnum-1][index].loc4;
   ...
   for (int i = 0; i < Pnum; ++i)
   {
      ...
      double diff3 =Pbar0.loc3-Psupp[i][indices[i]].loc3;
      double diff4 =Pbar0.loc4-Psupp[i][indices[i]].loc4;
      newc += lambda[i]*(diff1*diff1+diff2*diff2+diff3*diff3+diff4*diff4);
   }
   return newc;
}
